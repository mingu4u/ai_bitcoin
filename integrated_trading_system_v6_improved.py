...
def adjust_tp_sl_based_on_levels(symbol, action, current_price, original_sl, original_tp, market_data):
    """
    매물대 및 지지/저항선 기반으로 TP/SL을 현실적으로 조정
    ...
    try:
        ...
        if action == 'buy':
            ...
            # 1. TP가 너무 멀 경우 (10% 이상)
            if tp_distance_percent > 10:
                # 다음 저항선 찾기
                next_resistance = None
                
                for resistance in volume_profile_4h['resistance_levels']:
                    if resistance > current_price:
                        # 현실적인 범위 내 (6% 이내)
                        if (resistance - current_price) / current_price * 100 <= 6:
                            next_resistance = resistance
                            break
                
                if next_resistance:
                    adjusted_tp = next_resistance * 0.995  # 저항선 직전
                    tp_reason = f"저항선 ({next_resistance:.2f}) 직전으로 조정"
                    is_adjusted = True
                else:
                    # 저항선이 없으면 ATR 기반 (3-4 ATR)
                    adjusted_tp = current_price + (atr_hourly * 3.5)
                    tp_reason = "3.5xATR 목표가로 조정 (현실적 범위)"
                    is_adjusted = True
            
            # 2. TP가 너무 가까운 경우 (<1.5%)
            elif tp_distance_percent < 1.5:
                # 최소 2 ATR은 확보
                adjusted_tp = current_price + (atr_hourly * 2)
                tp_reason = "2xATR 최소 목표 확보"
                is_adjusted = True
            ...
            if tp_distance_percent > 10:
                ...
                for support in volume_profile_4h['support_levels']:
                    if support < current_price:
                        if (current_price - support) / current_price * 100 <= 6:
                            next_support = support
                            break
                
                if next_support:
                    adjusted_tp = next_support * 1.005  # 지지선 직후
                    tp_reason = f"지지선 ({next_support:.2f}) 직후로 조정"
                    is_adjusted = True
                else:
                    adjusted_tp = current_price - (atr_hourly * 3.5)
                    tp_reason = "3.5xATR 목표가로 조정"
                    is_adjusted = True
...
def ai_validate_signal(symbol, action, market_data, recent_trades_df, message_data=None):
    ...
    try:
        ...
        if action in ['buy', 'sell']:
            ...
            # 리스크가 high이면 즉시 거부
            if overbought_oversold_risk['is_risky'] and overbought_oversold_risk['risk_level'] == 'high':

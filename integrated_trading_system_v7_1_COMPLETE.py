...
def ai_validate_signal(symbol, action, market_data, recent_trades_df, message_data=None):
    """
    🆕 v7.1 AI를 사용하여 거래 신호를 검증 - 강화된 반대 진입 판단
    Pydantic 검증 및 에러 처리 강화
    """
...
⚠️ **CRITICAL: The original {action.upper()} signal appears to be WRONG DIRECTION!**

**Current Analysis:**
- Original Signal: {action.upper()}
- Recommended Signal: {'SELL' if action == 'buy' else 'BUY'} (opposite)
- Market Condition: EXTREME {'OVERBOUGHT' if action == 'buy' else 'OVERSOLD'}
- Risk Level: {overbought_oversold_risk['risk_level'].upper()}

**Detected Extreme Signals ({len(reverse_signals)}):**
{chr(10).join(['  🔴 ' + sig for sig in reverse_signals])}
...
"""
            ...
            # 리스크가 extreme이면 강한 경고 (반대 진입 권고)
            elif overbought_oversold_risk['risk_level'] == 'extreme':
                logger.warning(f"⛔ 과매수/과매도 리스크 EXTREME - 강력한 반대 진입 권고")
                logger.warning(f"   경고: {', '.join(overbought_oversold_risk['warnings'])}")
            
            # 리스크가 high이면서 반대 진입 기회가 없으면 거부
            elif overbought_oversold_risk['is_risky'] and overbought_oversold_risk['risk_level'] == 'high':
...
**🆕 v7.1 OVERBOUGHT/OVERSOLD ANALYSIS (CRITICAL!):**
═══════════════════════════════════════════
{f'''→ Risk Analysis:
  • Total Risk Score: {overbought_oversold_risk['total_risk_score']}
  • Risk Level: {overbought_oversold_risk['risk_level'].upper()}
  • Reverse Opportunity: {'🔴 YES - STRONGLY CONSIDER REVERSE!' if overbought_oversold_risk.get('reverse_opportunity', False) else '✅ NO'}
  • Reverse Strength: {overbought_oversold_risk.get('reverse_strength', 0):.1%} ({overbought_oversold_risk.get('reverse_strength_points', 0)}/15 points)

→ Key Indicator Values:
  • RSI (15m/1h/4h): {overbought_oversold_risk['scores'].get('rsi_15m', 50):.1f} / {overbought_oversold_risk['scores'].get('rsi_1h', 50):.1f} / {overbought_oversold_risk['scores'].get('rsi_4h', 50):.1f}
  • Stochastic (15m/1h): {overbought_oversold_risk['scores'].get('stoch_15m', 50):.1f} / {overbought_oversold_risk['scores'].get('stoch_1h', 50):.1f}
  • Williams %R (15m): {overbought_oversold_risk['scores'].get('williams_15m', -50):.1f}
  • CMF (15m/1h): {overbought_oversold_risk['scores'].get('cmf_15m', 0):.2f} / {overbought_oversold_risk['scores'].get('cmf_1h', 0):.2f}

→ Warnings:
{chr(10).join(['  🔴 ' + w for w in overbought_oversold_risk.get('warnings', [])]) if overbought_oversold_risk.get('warnings') else '  ✅ No warnings'}

→ Detected Reverse Signals:
{chr(10).join(['  ⚠️ ' + s for s in overbought_oversold_risk.get('reverse_signals', [])]) if overbought_oversold_risk.get('reverse_signals') else '  ✅ No extreme signals'}

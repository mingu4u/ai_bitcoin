...
def record_position_closure_with_real_pnl(symbol, position_data, close_type='manual'):
    """
    🆕 v8.1: 포지션 종료 시 바이낸스 실제 수익률을 기록하고 이벤트 발생
    
    close_type: 'manual', 'auto', 'sl', 'tp', 'auto_tpsl', 'liquidation'
    """
    try:
        # 1. 바이낸스에서 실제 PnL 가져오기
        ...
        if real_pnl_data:
            ...
            logger.warning(f"⚠️ {symbol} 바이낸스 PnL 조회 실패, 계산값 사용: ${realized_pnl:.2f}")
        ...
        """, (
            ...
            position_type, realized_pnl if real_pnl_data else None
        ))
        ...
        event_data = {
            ...
            'side': side,
            'is_binance_confirmed': bool(real_pnl_data)
        }
        
        trigger_dashboard_event('position_closed', symbol, event_data)
        
        logger.info(f"📊 {symbol} 포지션 종료 기록 완료 (실제 PnL: ${realized_pnl:.2f})")

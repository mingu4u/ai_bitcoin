...
# ⚙️ AI 모니터링 초기 자동 시작 여부
# False: 봇 시작 시 AI 모니터링 스레드를 자동 실행하지 않음 (대시보드/API에서 수동 시작)
# True: 기존 동작 (시작 시 자동 실행)
AI_MONITORING_AUTOSTART = False

# ⚙️ AI 검증(validation) 초기 디폴트
# 심볼별 ai_validation 플래그와 별개로, 전역 마스터 스위치 역할.
# False면 심볼별 설정과 무관하게 AI 검증을 건너뜀 (초기 안전 시작)
AI_VALIDATION_ENABLED = False

# 🆕 TP/SL 자동생성 옵션
# True: 웹훅 TP/SL이 null이면 봇이 자동 생성 (기존 동작)
# False: 웹훅 TP/SL이 null이면 TP/SL 없이 진입 (TradingView 종료 신호에 의존)
# ⚙️ 초기 디폴트: OFF (대시보드에서 수동 활성화)
AUTO_TP_SL_GENERATION = False

# ============ 🆕 v7.8: Emergency Drawdown Protection (긴급 낙폭 보호) ============
# TradingView 전략에서 손절가를 끈 경우의 안전장치
EMERGENCY_DRAWDOWN_ENABLED = False       # ⚙️ 초기 디폴트 OFF (대시보드에서 변경 가능)
...
# ============ 🆕 v7.9 Market Shield (멀티소스 융합 방어) ============
MARKET_SHIELD_ENABLED = False             # ⚙️ 초기 디폴트 OFF (대시보드에서 변경 가능)
MARKET_SHIELD_ENTRY_BLOCK_BEFORE = 120    # 이벤트 전 차단 시간 (분)
MARKET_SHIELD_ENTRY_BLOCK_AFTER = 60      # 이벤트 후 차단 시간 (분)
MARKET_SHIELD_ZSCORE_THRESHOLD = 4.0      # Z-Score 이상치 임계값
MARKET_SHIELD_ANOMALY_COOLDOWN = 900      # 이상 감지 후 진입 차단 시간 (초, 15분)

# Market Shield 런타임 변수
market_shield_running = False
market_shield_thread = None
market_shield_calendar_cache = []         # 당일 경제 일정 캐시
market_shield_calendar_date = None        # 캐시 날짜
market_shield_anomaly_flag = {}           # {symbol: {'timestamp': datetime, 'direction': 'drop'/'surge', 'z_price': float, 'z_volume': float}}
market_shield_entry_blocked = {}          # {symbol: {'until': datetime, 'reason': str}}
...
# ============================================================================
# 🆕 v7.9: 미등록 심볼 자동 등록 (동적 추가)
# ============================================================================
# TradingView Alert로 SYMBOL_CONFIG에 없는 심볼이 들어오면 아래 기본 설정으로
# 자동 등록 후 정상 처리한다. (기존 v7.8: 400 에러 반환 후 신호 폐기)
# ⚠️ 메모리 상 등록이므로 봇 재시작 시 초기화됨 → 검증 후 소스에 영구 반영 권장

# 자동 등록 기능 ON/OFF (환경변수로 제어 가능)
AUTO_REGISTER_SYMBOL = os.getenv('AUTO_REGISTER_SYMBOL', 'true').lower() == 'true'
# 거래소 상장 여부 검증 (오타/폐지 심볼 등록 방지)
AUTO_REGISTER_VERIFY_MARKET = os.getenv('AUTO_REGISTER_VERIFY_MARKET', 'true').lower() == 'true'
# 자동 등록 최대 심볼 수 (무한 증식 방지)
AUTO_REGISTER_MAX_SYMBOLS = int(os.getenv('AUTO_REGISTER_MAX_SYMBOLS', 50))

# 자동 등록 시 적용할 기본 설정 템플릿
DEFAULT_SYMBOL_CONFIG_TEMPLATE = {
    'leverage': 10,
    'position_size_percent': 40,
    'min_position_size': 10,
    'max_position_size': 100000,
...
# 자동 등록된 심볼 추적 (수동 등록분과 구분 — 상태 조회/로그용)
auto_registered_symbols = set()


# ============================================================================
# 🆕 v8.0: 리페인팅 방어 (Anti-Repaint Guard)
# ============================================================================
# 문제: Pine 실시간 종료(strategy.exit 스탑/리밋)가 리페인팅되면
#       진입 → 즉시 종료 → 즉시 재진입이 반복되어 수수료만 소진됨.
# 방어 2단:
#   1단 - 진입 직후 MIN_HOLD_SECONDS 이내의 종료 신호는 무시 (리페인팅 의심)
#   2단 - 1단이 발동한 심볼은 REPAINT_BLOCK_MINUTES 동안 신규 진입도 차단
#         (같은 봉에서 되살아난 진입 신호 = 리페인팅 확정으로 간주)

REPAINT_GUARD_ENABLED   = os.getenv('REPAINT_GUARD_ENABLED', 'true').lower() == 'true'
MIN_HOLD_SECONDS        = int(os.getenv('MIN_HOLD_SECONDS', 90))       # 진입 후 종료 무시 구간
REPAINT_BLOCK_MINUTES   = int(os.getenv('REPAINT_BLOCK_MINUTES', 60))  # 재진입 차단 시간
REPAINT_STRIKE_LIMIT    = int(os.getenv('REPAINT_STRIKE_LIMIT', 2))    # 누적 N회 시 장기 차단
ANOMALY_LOG_MAX         = 300                                          # 메모리 보관 최대 건수

# 심볼별 리페인팅 의심 상태: {symbol: {'until': datetime, 'strikes': int, 'last': datetime}}
repaint_suspects = {}
# 이상 현상 로그 (메모리 — 대시보드/텔레그램용). DB에도 병행 기록
anomaly_log = deque(maxlen=ANOMALY_LOG_MAX)
anomaly_lock = threading.Lock()


def init_anomaly_table():
    """🆕 v8.0: 이상 현상 로그 테이블 생성"""
    try:
        conn = get_db_connection()
        c = conn.cursor()
        c.execute('''
        ...
        ''')
        c.execute('CREATE INDEX IF NOT EXISTS idx_anomaly_ts ON anomaly_log(timestamp)')
        c.execute('CREATE INDEX IF NOT EXISTS idx_anomaly_symbol ON anomaly_log(symbol)')
        conn.commit()
        conn.close()
        logger.info("✅ anomaly_log 테이블 준비 완료")
    except Exception as e:
        logger.error(f"❌ anomaly_log 테이블 생성 실패: {e}")


def record_anomaly(symbol, anomaly_type, detail, action_taken='',
                   severity='warning', held_seconds=None, strikes=0, notify=True):
    """
    🆕 v8.0: 이상 현상 기록 (메모리 + DB + 텔레그램)

    anomaly_type 예시:
      repaint_close_ignored  : 진입 직후 종료 신호 무시
      repaint_entry_blocked  : 리페인팅 의심 심볼의 재진입 차단
      repaint_confirmed      : 무시 직후 재진입 신호 → 리페인팅 확정
    """
    entry = {
    ...
    try:
        ...
        c.execute('''INSERT INTO anomaly_log
                     (timestamp, symbol, anomaly_type, severity, detail, action_taken, held_seconds, strikes)
                     VALUES (?, ?, ?, ?, ?, ?, ?, ?)''',
                  (entry['timestamp'], symbol, anomaly_type, severity, detail,
                   action_taken, entry['held_seconds'], strikes))
        conn.commit()
        conn.close()
    except Exception as e:
        logger.error(f"❌ 이상 현상 DB 기록 실패: {e}")

    logger.warning(f"⚠️ [이상감지] {symbol} | {anomaly_type} | {detail} → {action_taken}")

    # 텔레그램 알림
    if notify and ENABLE_TELEGRAM:
        icon = '🔁' if 'repaint' in anomaly_type else '⚠️'
        msg = f"""
{icon} <b>이상 현상 감지</b>
...
"""
        if held_seconds is not None:
            msg += f"<b>보유 시간:</b> {held_seconds:.1f}초\n"
        if strikes:
            msg += f"<b>누적 감지:</b> {strikes}회\n"
        msg += f"\n⏰ {entry['timestamp']}"
        send_telegram_notification(msg.strip(), 'warning')

    return entry


def get_position_age_seconds(symbol):
    """포지션 진입 후 경과 시간(초). 포지션 없거나 시간 불명이면 None"""
    ...
    try:
        ...
        return (datetime.now() - et).total_seconds()
    except Exception:
        return None


def should_ignore_close_signal(symbol, action):
    """
    🆕 v8.0 [1단] 진입 직후 종료 신호를 리페인팅으로 판단해 무시할지 결정

    Returns: (ignore: bool, reason: str, age: float|None)
    """
    if not REPAINT_GUARD_ENABLED:
        return False, '', None

    age = get_position_age_seconds(symbol)
    if age is None:
        return False, '', None          # 포지션 정보 없음 → 정상 처리
    if age >= MIN_HOLD_SECONDS:
        return False, '', age           # 충분히 보유함 → 정상 종료

    return True, f'진입 후 {age:.1f}초 만에 {action} 신호 (최소 보유 {MIN_HOLD_SECONDS}초 미만)', age


def mark_repaint_suspect(symbol, reason=''):
    """1단 발동 심볼을 의심 목록에 등록 (스트라이크 누적)"""
    now = datetime.now()
    info = repaint_suspects.get(symbol, {'strikes': 0})
    info['strikes'] = info.get('strikes', 0) + 1
    # 스트라이크가 쌓이면 차단 시간 연장 (2회=2배, 3회=3배 ...)
    block_min = REPAINT_BLOCK_MINUTES * max(1, info['strikes'] // max(1, REPAINT_STRIKE_LIMIT) + 1) \
                if info['strikes'] >= REPAINT_STRIKE_LIMIT else REPAINT_BLOCK_MINUTES
    info['until'] = now + timedelta(minutes=block_min)
    info['last'] = now
    info['reason'] = reason
    repaint_suspects[symbol] = info
    return info


def is_repaint_blocked(symbol):
    """
    🆕 v8.0 [2단] 리페인팅 의심 심볼의 신규 진입 차단 여부

    Returns: (blocked: bool, reason: str, info: dict|None)
    """
    if not REPAINT_GUARD_ENABLED:
        return False, '', None

    info = repaint_suspects.get(symbol)
    if not info:
        return False, '', None

    until = info.get('until')
    if until and datetime.now() >= until:
        # 차단 기간 종료 → 해제 (스트라이크는 유지해 재발 시 더 길게 차단)
        info.pop('until', None)
        repaint_suspects[symbol] = info
        logger.info(f"✅ 리페인팅 차단 해제: {symbol} (누적 {info.get('strikes', 0)}회)")
        return False, '', info

    if until:
        remain = (until - datetime.now()).total_seconds() / 60
        return True, f'리페인팅 의심 차단 중 (누적 {info.get("strikes", 0)}회, 잔여 {remain:.0f}분)', info

    return False, '', info


def verify_symbol_on_exchange(symbol: str) -> bool:
    """
    🆕 v7.9: 바이낸스 선물에 해당 심볼이 실제 상장되어 있는지 확인

    Args:
        symbol: 정규화된 심볼 (예: 'NEAR/USDT')

    Returns:
        bool: 거래 가능하면 True (검증 불가 시에도 True — 등록 자체를 막지 않음)
    """
    try:
        if exchange is None:
            logger.warning("⚠️ 거래소 객체 없음 - 상장 검증 생략")
            return True

        if not exchange.markets:
            exchange.load_markets()

        # ccxt 선물 심볼 형식: 'NEAR/USDT:USDT'
        candidates = [f"{symbol}:USDT", symbol]
        for cand in candidates:
            market = exchange.markets.get(cand)
            if market and market.get('active', True):
                return True

        logger.warning(f"⚠️ {symbol}이 거래소 마켓 목록에 없음")
        return False

    except Exception as e:
        # 검증 실패는 등록 차단 사유가 아님 (네트워크 오류 등으로 신호를 잃지 않도록)
        logger.warning(f"⚠️ 상장 검증 중 오류 (등록은 진행): {e}")
        return True


def register_symbol_dynamically(symbol: str, original_symbol: str = None) -> tuple:
    """
    🆕 v7.9: 미등록 심볼을 기본 설정으로 SYMBOL_CONFIG에 동적 추가

    Args:
        symbol: 정규화된 심볼 (예: 'NEAR/USDT')
        original_symbol: TradingView 원본 심볼 (로그용)

    Returns:
        tuple: (성공 여부, 사유 메시지)
    """
    global SYMBOL_CONFIG, auto_registered_symbols

    normalized = normalize_symbol(symbol)
    original_symbol = original_symbol or symbol

    # 이미 등록되어 있으면 성공 처리
    if normalized in SYMBOL_CONFIG:
        return True, "이미 등록됨"

    # 기능 비활성화 상태
    if not AUTO_REGISTER_SYMBOL:
        return False, "자동 등록 비활성화 (AUTO_REGISTER_SYMBOL=false)"

    # 자동 등록 상한 확인
    if len(auto_registered_symbols) >= AUTO_REGISTER_MAX_SYMBOLS:
        return False, f"자동 등록 상한 초과 ({AUTO_REGISTER_MAX_SYMBOLS}개)"

    # 거래소 상장 검증
    if AUTO_REGISTER_VERIFY_MARKET and not verify_symbol_on_exchange(normalized):
        return False, "거래소 미상장 또는 거래 불가 심볼"

    # 등록 실행 (템플릿 복사 — 원본 오염 방지)
    SYMBOL_CONFIG[normalized] = DEFAULT_SYMBOL_CONFIG_TEMPLATE.copy()
    auto_registered_symbols.add(normalized)

    logger.info(f"🆕 심볼 자동 등록 완료: {original_symbol} → {normalized}")
    logger.info(f"   설정: 레버리지 {DEFAULT_SYMBOL_CONFIG_TEMPLATE['leverage']}x, "
                f"진입비중 {DEFAULT_SYMBOL_CONFIG_TEMPLATE['position_size_percent']}%, "
                f"AI검증 {DEFAULT_SYMBOL_CONFIG_TEMPLATE['ai_validation']}, "
                f"AI모니터링 {DEFAULT_SYMBOL_CONFIG_TEMPLATE['ai_monitoring']}")
    logger.info(f"   자동 등록 누적: {len(auto_registered_symbols)}/{AUTO_REGISTER_MAX_SYMBOLS}개")

    return True, "자동 등록 성공"
                ...
                if symbol in current_positions:
                    ...
                    if not is_configured:
                        ...
                        SYMBOL_CONFIG[symbol] = {
                            ...
                            'ai_monitoring': False,  # ⚙️ 초기 디폴트 OFF (대시보드에서 수동 활성화)
...
def market_shield_fetch_calendar():
    """
    Layer 1: 당일 주요 경제 이벤트 조회 (하루 1회)
    FinancialModelingPrep 무료 API 사용 (실패 시 DeepSeek로 fallback)
    """
    global market_shield_calendar_cache, market_shield_calendar_date
    
    today = datetime.now().strftime('%Y-%m-%d')
    if market_shield_calendar_date == today and market_shield_calendar_cache:
        # 🆕 캐시된 이벤트도 매번 과거 필터링 (시간이 지나면서 이벤트가 만료될 수 있음)
        now = datetime.now()
        still_valid = [e for e in market_shield_calendar_cache 
                       if e['time'] + timedelta(minutes=MARKET_SHIELD_ENTRY_BLOCK_AFTER) > now]
        market_shield_calendar_cache = still_valid
        return still_valid
    
    HIGH_IMPACT_KEYWORDS = [
        'CPI', 'PPI', 'FOMC', 'Fed', 'Interest Rate', 'Non-Farm', 'NFP',
        'GDP', 'PCE', 'Unemployment', 'Jackson Hole', 'Powell', 'Yellen',
        'ECB', 'BOJ', 'BOE', 'Retail Sales', 'ISM', 'PMI', 'Core Inflation'
    ]
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            for item in data:
                ...
                is_high = impact == 'High' or any(kw.lower() in event_name.lower() for kw in HIGH_IMPACT_KEYWORDS)
                
                if is_high and event_time_str:
                    try:
            ...
            if events:
                logger.info(f"📅 Market Shield: {len(events)}개 주요 경제 이벤트 로드")
    except Exception as e:
        logger.warning(f"📅 Calendar API 실패: {e}")
    
    # 방법 2: DeepSeek fallback
    if not events:
        try:
            from openai import OpenAI
            client = OpenAI(api_key=os.getenv("DEEPSEEK_API_KEY"), base_url="https://api.deepseek.com")
            if client.api_key:
                ...
                event_list = result if isinstance(result, list) else result.get('events', [])
                for item in event_list:
                    try:
                        ...
                        pass
                if events:
                    logger.info(f"📅 Market Shield (DeepSeek): {len(events)}개 이벤트")
        except Exception as e:
            logger.warning(f"📅 DeepSeek calendar fallback 실패: {e}")
    
    # 🆕 v7.9: 이미 지난 이벤트 필터링 (현재 시간 + 30분 여유 이전은 제거)
    now = datetime.now()
    future_events = []
    for e in events:
        event_time = e['time']
        ...
        # 이벤트 시간 + 차단 후 시간이 아직 안 지났으면 유지
        if event_time + timedelta(minutes=MARKET_SHIELD_ENTRY_BLOCK_AFTER) > now:
            future_events.append(e)
        else:
            logger.info(f"📅 과거 이벤트 제거: {e['name']} @ {event_time}")
    
    events = future_events
    
    market_shield_calendar_cache = events
    market_shield_calendar_date = today
    return events


def market_shield_check_calendar_block():
    """
    Layer 1: 현재 시간이 주요 경제 이벤트 ±차단 범위 내인지 확인
    Returns: {'blocked': bool, 'reason': str, 'event': str, 'unblock_time': datetime}
    """
    ...
    for event in events:
        ...
        # 시간대 차이 보정 (naive datetime이면 그대로 비교)
        if hasattr(event_time, 'tzinfo') and event_time.tzinfo:
            now_aware = datetime.now(event_time.tzinfo)
        else:
            now_aware = now
        
        block_start = event_time - timedelta(minutes=MARKET_SHIELD_ENTRY_BLOCK_BEFORE)
        block_end = event_time + timedelta(minutes=MARKET_SHIELD_ENTRY_BLOCK_AFTER)
        
        if block_start <= now_aware <= block_end:
            return {
                'blocked': True,
                'reason': f"경제 이벤트 차단: {event['name']} ({event['country']})",
                'event': event['name'],
                'event_time': event_time,
                'unblock_time': block_end
            }
    
    return {'blocked': False}


def market_shield_check_anomaly():
    """
    Layer 2: 모든 활성 심볼의 가격/거래량 Z-Score 이상 탐지 (1분마다)
    바이낸스 15분봉 24시간 데이터 기준 동적 계산
    """
    global market_shield_anomaly_flag, market_shield_entry_blocked
    
    if not MARKET_SHIELD_ENABLED:
        return
    
    # 모니터링 대상: current_positions + SYMBOL_CONFIG 활성 심볼
    symbols_to_check = set(current_positions.keys())
    for s, c in SYMBOL_CONFIG.items():
        if c.get('enabled', True):
            symbols_to_check.add(s)
    ...
    for symbol in symbols_to_check:
        try:
            ...
            df['vol_change'] = df['volume']
            
            df = df.dropna()
            if len(df) < 30:
                continue
            
            # Z-Score 계산 (최근 값 vs 24시간 분포)
            latest_price_change = df['price_change'].iloc[-1]
            price_mean = df['price_change'].iloc[:-1].mean()
            price_std = df['price_change'].iloc[:-1].std()
            
            latest_volume = df['volume'].iloc[-1]
            vol_mean = df['volume'].iloc[:-1].mean()
            vol_std = df['volume'].iloc[:-1].std()
            
            if price_std == 0 or vol_std == 0:
                continue
            
            z_price = abs(latest_price_change - price_mean) / price_std
            z_volume = (latest_volume - vol_mean) / vol_std
            
            # 이상치 판정: 가격 Z > 3σ AND 거래량 Z > 3σ
            if z_price > MARKET_SHIELD_ZSCORE_THRESHOLD and z_volume > MARKET_SHIELD_ZSCORE_THRESHOLD:
                direction = 'surge' if latest_price_change > 0 else 'drop'
                
                # 이미 감지된 이상이면 스킵 (쿨다운)
                if symbol in market_shield_anomaly_flag:
                    last_detected = market_shield_anomaly_flag[symbol]['timestamp']
                    if (datetime.now() - last_detected).total_seconds() < MARKET_SHIELD_ANOMALY_COOLDOWN:
                        continue
                
                market_shield_anomaly_flag[symbol] = {
                ...
                # 신규 진입 차단
                market_shield_entry_blocked[symbol] = {
                    'until': datetime.now() + timedelta(seconds=MARKET_SHIELD_ANOMALY_COOLDOWN),
                    'reason': f"이상 감지: {direction} (Z_price={z_price:.1f}σ, Z_vol={z_volume:.1f}σ, Δ{latest_price_change:+.2f}%)"
                }
                
                logger.warning(f"🛡️ MARKET SHIELD: {symbol} 이상 감지! direction={direction}, "
                              f"Z_price={z_price:.1f}σ, Z_vol={z_volume:.1f}σ, Δ{latest_price_change:+.2f}%")
                
                if ENABLE_TELEGRAM:
                    send_telegram_notification(
                        f"🛡️ <b>Market Shield - 이상 감지</b>\n\n"
                        f"<b>심볼:</b> {symbol}\n"
                        f"<b>방향:</b> {'📈 급등' if direction == 'surge' else '📉 급락'}\n"
                        f"<b>15분 변동:</b> {latest_price_change:+.2f}%\n"
                        f"<b>가격 Z-Score:</b> {z_price:.1f}σ\n"
                        f"<b>거래량 Z-Score:</b> {z_volume:.1f}σ\n"
                        f"<b>신규 진입:</b> {MARKET_SHIELD_ANOMALY_COOLDOWN//60}분간 차단\n\n"
                        f"🔍 원인 분석 중...\n"
                        f"⏰ {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
                        'warning'
                    )
                
                # Layer 3+4 발동: 원인 확인 + 포지션 결정
                market_shield_analyze_and_decide(symbol, direction, latest_price_change, z_price, z_volume)
            
            time.sleep(0.3)
            
        except Exception as e:
            logger.error(f"Market Shield 이상 탐지 오류 ({symbol}): {e}")


def market_shield_analyze_and_decide(symbol, direction, price_change, z_price, z_volume):
    """
    Layer 3+4: SERP 뉴스 검색 → DeepSeek 감성 분석 → 포지션 청산 여부 결정
    이벤트 감지 시에만 호출
    """
    try:
        from openai import OpenAI
        ...
        try:
            ...
            search_queries = [f"{base_symbol} crypto news today", "crypto market crash news", "bitcoin breaking news"]
            
            serp_api_key = os.getenv("SERP_API_KEY") or os.getenv("SERPAPI_API_KEY")
            if serp_api_key:
                for query in search_queries[:2]:  # 비용 절약: 2개만
                    try:
                        resp = requests.get(
                            "https://serpapi.com/search",
                            params={"q": query, "tbm": "nws", "num": 3, "api_key": serp_api_key},
                            timeout=10
                        )
                        if resp.status_code == 200:
                            results = resp.json().get('news_results', [])
                            for r in results[:3]:
                                news_headlines.append(f"{r.get('title', '')} - {r.get('source', '')}")
                    except:
                        pass
        except Exception as e:
            logger.warning(f"SERP 뉴스 검색 실패: {e}")
        
        if not news_headlines:
            news_headlines = ["No recent news found"]
        
        # ========== Layer 3+4: DeepSeek 감성 분석 + 포지션 결정 ==========
        client = OpenAI(api_key=os.getenv("DEEPSEEK_API_KEY"), base_url="https://api.deepseek.com")
        if not client.api_key:
            logger.warning("Market Shield: DeepSeek API key missing")
            return
        ...
        if symbol in current_positions:
            ...
            entry_price = pos.get('entry_price', 0)
            position_info = f"Current position: {side.upper()} from ${entry_price:.4f}"
        else:
            position_info = "No current position"
        
        prompt = f"""Crypto market anomaly detected. Analyze and decide.

EVENT:
- Symbol: {symbol}
- Direction: {direction} ({price_change:+.2f}% in 5min)
- Price Z-Score: {z_price:.1f}σ (extreme)
- Volume Z-Score: {z_volume:.1f}σ (extreme)
...
TASK:
1. Analyze the likely cause (economic data, war/geopolitics, exchange hack, whale manipulation, etc.)
2. Assess if the move will continue or reverse
3. If holding a position AGAINST the move direction → recommend close
4. If holding a position WITH the move direction or no position → recommend hold

OUTPUT (JSON only):
{{
    "sentiment": "bearish" | "bullish" | "neutral",
    "cause": "<likely cause in 1 sentence>",
    "severity": "critical" | "high" | "medium" | "low",
    "position_action": "close" | "hold",
    "reason": "<1 sentence decision reason>"
}}"""

        response = client.chat.completions.create(
            model="deepseek-chat",
            messages=[
                {"role": "system", "content": "You are a crypto market analyst. Analyze the anomaly and decide whether to close the position. Return ONLY JSON."},
                {"role": "user", "content": prompt}
            ],
            response_format={'type': 'json_object'},
        ...
        action = result.get('position_action', 'hold')
        reason = result.get('reason', '')
        
        logger.info(f"🛡️ Market Shield 분석 완료: {symbol} - sentiment={sentiment}, severity={severity}, action={action}")
        
        # 텔레그램 알림
        sentiment_emoji = {'bearish': '🔴', 'bullish': '🟢', 'neutral': '⚪'}.get(sentiment, '⚪')
        severity_emoji = {'critical': '🚨', 'high': '⚠️', 'medium': '📊', 'low': 'ℹ️'}.get(severity, '📊')
        
        if ENABLE_TELEGRAM:
            send_telegram_notification(
                f"🛡️ <b>Market Shield - 분석 결과</b>\n\n"
                f"<b>심볼:</b> {symbol}\n"
                f"<b>감성:</b> {sentiment_emoji} {sentiment.upper()}\n"
                f"<b>심각도:</b> {severity_emoji} {severity.upper()}\n"
                f"<b>원인:</b> {cause}\n"
                f"<b>포지션:</b> {position_info}\n"
                f"<b>결정:</b> {'🔴 청산' if action == 'close' else '🟢 유지'}\n"
                f"<b>사유:</b> {reason}\n\n"
                f"⏰ {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
                'error' if action == 'close' else 'info'
            )
        
        # ========== Layer 4: 포지션 청산 실행 ==========
        if action == 'close' and symbol in current_positions:
            logger.warning(f"🛡️ Market Shield: {symbol} 포지션 청산 실행!")
            try:
                success_count = close_position_for_all_users(symbol)
                if success_count > 0:
                    if symbol in current_positions:
                        record_completed_trade_with_binance(symbol, current_positions[symbol],
                                                           close_reason=f'market_shield_{direction}')
                        del current_positions[symbol]
                    
                    if ENABLE_TELEGRAM:
                        send_telegram_notification(
                            f"🛡️ <b>Market Shield 청산 완료</b>\n\n"
                            f"<b>심볼:</b> {symbol}\n"
                            f"<b>사유:</b> {cause}\n"
                            f"<b>청산:</b> {success_count}/{len(exchanges)}명\n"
                            f"⏰ {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
                            'success'
                        )
            except Exception as e:
                logger.error(f"Market Shield 청산 실패: {e}")
    
    except Exception as e:
        logger.error(f"Market Shield Layer 3+4 오류: {e}", exc_info=True)


def market_shield_is_entry_blocked(symbol):
    """
    웹훅 진입 시 호출: 경제 캘린더 또는 이상 감지로 진입 차단 여부 확인
    ...
    # Layer 1: 경제 캘린더 체크
    cal_check = market_shield_check_calendar_block()
    if cal_check['blocked']:
        return cal_check
    
    # Layer 2: 이상 감지 차단 체크
    if symbol in market_shield_entry_blocked:
        block_info = market_shield_entry_blocked[symbol]
        if datetime.now() < block_info['until']:
            return {
                'blocked': True,
                'reason': block_info['reason'],
                'unblock_time': block_info['until']
            }
        else:
            # 차단 만료 → 해제
            del market_shield_entry_blocked[symbol]
    
    return {'blocked': False}


def start_market_shield():
    """Market Shield 백그라운드 스레드 시작"""
    ...
    def shield_loop():
        ...
        try:
            events = market_shield_fetch_calendar()
            # 🆕 텔레그램 표시용: 이벤트 시간 자체가 아직 안 지난 것만 표시
            now = datetime.now()
        ...
        while market_shield_running:
            try:
                ...
                logger.error(f"Market Shield 루프 오류: {e}", exc_info=True)
                time.sleep(30)
    
    if not market_shield_running:
        market_shield_thread = threading.Thread(target=shield_loop, daemon=True)
        market_shield_thread.start()


def stop_market_shield():
    """Market Shield 중지"""
    global market_shield_running
    market_shield_running = False
    logger.info("🛡️ Market Shield 중지")
...
def place_orders_with_sl_tp(symbol, action, amount, stop_loss_price, take_profit_price, 
    ...
    try:
        ...
        # 결과 로깅
        if sl_order is None and adjusted_sl is not None:
            logger.error(f"실패 상세 - SL가격: ${adjusted_sl:.4f}, 현재가: ${current_price:.4f}, 수량: {amount:.6f}")
        if tp_order is None and adjusted_tp is not None:
...
def format_position_entry_message(symbol, action, amount, entry_price, sl, tp, pl_ratio, 
    ...
    if sl is not None:
        ...
        risk_lines.append(f"• 스탑로스: ${sl:,.4f} (예상 손실: ${abs(potential_loss):,.2f})")
    else:
        risk_lines.append("• 스탑로스: 미설정")
    ...
    if tp is not None:
        ...
        risk_lines.append(f"• 테이크프로핏: ${tp:,.4f} (예상 이익: ${abs(potential_profit):,.2f})")
    else:
        risk_lines.append("• 테이크프로핏: 미설정")
    
    if sl is not None and tp is not None and pl_ratio > 0:
        risk_lines.append(f"• P/L 비율: {pl_ratio:.2f}")
...
"""
    ...
    # 트레일링 스탑 표시 (trailing_activation이 0이면 "즉시 활성화")
    if trailing_stop is not None and trailing_stop > 0:
        message += f"""• 🔄 트레일링 스탑: {trailing_stop}% (바이낸스 TRAILING_STOP_MARKET)
"""
        if trailing_activation is not None and trailing_activation > 0:
            message += f"""• 활성화 기준: +{trailing_activation}%
"""
        else:
            message += f"""• 활성화 기준: 즉시 (activatePrice 미설정)
"""
...
def webhook():
    ...
    try:
        ...
        symbol_mapping = {
            ...
            'POPCATUSDT.P': 'POPCAT/USDT',    
            'HUSDT': 'H/USDT',
            'HUSDT.P': 'H/USDT',
            'SPXUSDT': 'SPX/USDT',
            'SPXUSDT.P': 'SPX/USDT',               
            'PLTRUSDT': 'PLTR/USDT',
            'PLTRUSDT.P': 'PLTR/USDT',
            'MEGAUSDT': 'MEGA/USDT',
            ...
            'MOODENGUSDT.P': 'MOODENG/USDT',
            'HEMIUSDT': 'HEMI/USDT',
            'HEMIUSDT.P': 'HEMI/USDT',
            'EPICUSDT': 'EPIC/USDT',
            'EPICUSDT.P': 'EPIC/USDT',
            'ROBOUSDT': 'ROBO/USDT',
            'ROBOUSDT.P': 'ROBO/USDT',          
        }
        ...
        # 🆕 v7.9: 미등록 심볼이면 기본 설정으로 자동 등록 후 계속 진행
        if not is_symbol_configured(symbol):
            registered, reason = register_symbol_dynamically(symbol, original_symbol)

            if not registered:
                error_msg = f'심볼 {symbol} 자동 등록 실패: {reason} (원본: {original_symbol})'
                logger.error(f"❌ {error_msg}")
...
⚠️ SYMBOL_CONFIG에 해당 심볼을 수동 추가하거나 설정을 확인해주세요.
...
⏰ {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
            ...
            # 자동 등록 성공 → 텔레그램 안내 후 정상 처리 계속
            logger.info(f"✅ 미등록 심볼 자동 등록 후 신호 처리 계속: {symbol}")
            if ENABLE_TELEGRAM:
                cfg = SYMBOL_CONFIG[normalize_symbol(symbol)]
                notify_msg = f"""
🆕 <b>신규 심볼 자동 등록</b>
...
<b>적용 설정 (기본값)</b>
• 레버리지: {cfg['leverage']}x
• 진입 비중: {cfg['position_size_percent']}%
• 최소/최대: {cfg['min_position_size']} ~ {cfg['max_position_size']}
• AI 검증: {'ON' if cfg['ai_validation'] else 'OFF'}
• AI 모니터링: {'ON' if cfg['ai_monitoring'] else 'OFF'}

자동 등록 누적: {len(auto_registered_symbols)}/{AUTO_REGISTER_MAX_SYMBOLS}개
⚠️ 메모리 등록이므로 재시작 시 초기화됩니다.

⏰ {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
                """.strip()
                send_telegram_notification(notify_msg, 'success')
        ...
        # ====================================================================
        # 🆕 v8.0 [리페인팅 방어 2단] 의심 심볼의 신규 진입 차단
        #   1단(종료 무시)이 발동한 직후 되살아난 진입 신호 = 리페인팅 확정.
        #   차단 기간 동안 해당 심볼의 buy/sell을 모두 무시해 수수료 소진을 막는다.
        # ====================================================================
        if action in ['buy', 'sell']:
            _blocked, _breason, _binfo = is_repaint_blocked(symbol)
            if _blocked:
                _strikes = (_binfo or {}).get('strikes', 0)
                record_anomaly(
                    symbol=symbol,
                    anomaly_type='repaint_entry_blocked',
                    detail=f'{action.upper()} 진입 신호 차단 — {_breason}',
                    action_taken='진입 무시 (리페인팅 재진입 루프 차단)',
                    severity='error' if _strikes >= REPAINT_STRIKE_LIMIT else 'warning',
                    strikes=_strikes,
                    notify=(_strikes <= REPAINT_STRIKE_LIMIT)  # 반복 알림 폭주 방지
                )
                return jsonify({
                    'status': 'blocked',
                    'reason': 'repaint_guard',
                    'detail': _breason,
                    'symbol': symbol,
                    'strikes': _strikes
                }), 200
        ...
        # 🆕 v7.9: Market Shield 진입 차단 체크 (buy/sell만, close는 통과)
        if MARKET_SHIELD_ENABLED and action in ['buy', 'sell']:
            shield_check = market_shield_is_entry_blocked(symbol)
            if shield_check['blocked']:
                logger.warning(f"🛡️ Market Shield 진입 차단: {symbol} {action} - {shield_check['reason']}")
                
                if ENABLE_TELEGRAM:
                    unblock = shield_check.get('unblock_time')
                    unblock_kst = (unblock + timedelta(hours=9)).strftime('%H:%M:%S KST') if unblock else '미정'
                    send_telegram_notification(
                        f"🛡️ <b>Market Shield 진입 차단</b>\n\n"
                        f"<b>심볼:</b> {symbol}\n"
                        f"<b>방향:</b> {action.upper()}\n"
                        f"<b>사유:</b> {shield_check['reason']}\n"
                        f"<b>해제 예정:</b> {unblock_kst}\n\n"
                        f"⏰ {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
                        'warning'
                    )
                ...
                return jsonify({
                    'status': 'blocked',
                    'reason': shield_check['reason'],
                    'source': 'market_shield'
                }), 200
        
        # ====================================================================
        # 🆕 v8.0 [리페인팅 방어 1단] 진입 직후 종료 신호 무시
        #   Pine 실시간 종료가 리페인팅되면 진입→즉시종료→즉시재진입이 반복되어
        #   수수료만 소진됨. 최소 보유 시간 미만의 종료 신호는 무시하고 기록한다.
        # ====================================================================
        if action in ('partial_close', 'close', 'close_position'):
            _ignore, _reason, _age = should_ignore_close_signal(symbol, action)
            if _ignore:
                _info = mark_repaint_suspect(symbol, _reason)
                record_anomaly(
                    symbol=symbol,
                    anomaly_type='repaint_close_ignored',
                    detail=_reason,
                    action_taken=f'종료 신호 무시 · 신규 진입 {REPAINT_BLOCK_MINUTES}분 차단',
                    severity='warning',
                    held_seconds=_age,
                    strikes=_info.get('strikes', 1)
                )
                return jsonify({
                    'status': 'ignored',
                    'reason': 'repaint_guard',
                    'detail': _reason,
                    'symbol': symbol,
                    'held_seconds': round(_age, 1) if _age is not None else None,
                    'blocked_until': _info.get('until').strftime('%Y-%m-%d %H:%M:%S') if _info.get('until') else None
                }), 200
        ...
        # AI 검증이 활성화되어 있는지 확인
        # ⚙️ 전역 마스터 스위치(AI_VALIDATION_ENABLED)가 OFF면 심볼 설정과 무관하게 건너뜀
        use_ai = AI_VALIDATION_ENABLED and symbol_config.get('ai_validation', False)
...
def get_anomalies():
    ...
    try:
        ...
        try:
            conn = get_db_connection()
            c = conn.cursor()
            q = "SELECT timestamp, symbol, anomaly_type, severity, detail, action_taken, held_seconds, strikes FROM anomaly_log WHERE 1=1"
            params = []
            if symbol_f:
                q += " AND symbol = ?"; params.append(symbol_f)
            if hours:
                q += " AND timestamp >= datetime('now', ?)"; params.append(f'-{int(hours)} hours')
            q += " ORDER BY id DESC LIMIT ?"; params.append(limit)
            c.execute(q, params)
            for r in c.fetchall():
                rows.append({
                    'timestamp': r[0], 'symbol': r[1], 'anomaly_type': r[2],
                    'severity': r[3], 'detail': r[4], 'action_taken': r[5],
                    'held_seconds': r[6], 'strikes': r[7]
                })
            conn.close()
        except Exception as e:
            logger.warning(f"⚠️ anomaly DB 조회 실패, 메모리 로그 사용: {e}")
            with anomaly_lock:
                rows = list(anomaly_log)[-limit:][::-1]

        # 현재 차단 중인 심볼 요약
        blocked = []
        now = datetime.now()
        for sym, info in repaint_suspects.items():
            until = info.get('until')
            blocked.append({
                'symbol': sym,
                ...
                'remain_minutes': round((until - now).total_seconds() / 60, 1) if (until and now < until) else 0,
                'reason': info.get('reason', '')
            })
        blocked.sort(key=lambda x: (not x['blocked'], -x['strikes']))

        return jsonify({
            'status': 'success',
            'guard_enabled': REPAINT_GUARD_ENABLED,
            'min_hold_seconds': MIN_HOLD_SECONDS,
            'block_minutes': REPAINT_BLOCK_MINUTES,
            'anomalies': rows,
            'total': len(rows),
            'suspects': blocked,
            'blocked_count': sum(1 for b in blocked if b['blocked'])
        }), 200
    except Exception as e:
        logger.error(f"❌ 이상현상 조회 오류: {e}")
        return jsonify({'status': 'error', 'error': str(e)}), 500


@app.route('/anomalies/unblock', methods=['POST'])
def unblock_symbol():
    """리페인팅 차단 수동 해제"""
    try:
        ...
        if clear_all:
            n = len(repaint_suspects)
            repaint_suspects.clear()
            logger.info(f"✅ 리페인팅 차단 전체 해제 ({n}개)")
            return jsonify({'status': 'success', 'cleared': n}), 200

        if not symbol:
            return jsonify({'status': 'error', 'error': 'symbol 필요'}), 400

        symbol = normalize_symbol(symbol)
        if symbol in repaint_suspects:
            repaint_suspects.pop(symbol)
            logger.info(f"✅ 리페인팅 차단 해제: {symbol}")
            return jsonify({'status': 'success', 'symbol': symbol}), 200
        return jsonify({'status': 'not_found', 'symbol': symbol}), 200
    except Exception as e:
        return jsonify({'status': 'error', 'error': str(e)}), 500


@app.route('/binance/realized-pnl', methods=['GET'])
def binance_realized_pnl():
    """
    🆕 v8.0: 바이낸스 실제 실현손익 조회 (심볼별)

    대시보드의 내부 DB 집계와 바이낸스 실제 손익이 불일치하는 문제 해결.
    거래소 income history를 직접 집계하므로 바이낸스 화면과 정확히 일치한다.
      REALIZED_PNL + COMMISSION + FUNDING_FEE = 순 실현손익
    """
    try:
        days = int(request.args.get('days', 30))
        if exchange is None:
            return jsonify({'status': 'error', 'error': '거래소 객체 없음'}), 500
        ...
        for itype in income_types:
            cursor_ms = start_ms
            guard = 0
            while guard < 20:   # 최대 20페이지 (페이지당 1000건)
                guard += 1
                try:
                    ...
                    logger.warning(f"⚠️ income 조회 실패({itype}): {e}")
                    break
                ...
                for r in rows:
                    ...
                    d = agg.setdefault(sym, {'realized_pnl': 0.0, 'commission': 0.0, 'funding_fee': 0.0})
                ...
                if len(rows) < 1000:
                    break
                cursor_ms = int(rows[-1].get('time', cursor_ms)) + 1

        result = []
        for sym, d in agg.items():
            net = d['realized_pnl'] + d['commission'] + d['funding_fee']  # 수수료·펀딩은 음수로 옴
            result.append({
                'symbol': sym,
        ...
        totals = {
            'realized_pnl': round(sum(r['realized_pnl'] for r in result), 4),
            'commission': round(sum(r['commission'] for r in result), 4),
            'funding_fee': round(sum(r['funding_fee'] for r in result), 4),
            'net_pnl': round(sum(r['net_pnl'] for r in result), 4)
        }
        ...
        logger.error(f"❌ 바이낸스 실현손익 조회 오류: {e}")
        return jsonify({'status': 'error', 'error': str(e)}), 500
...
def status():
    ...
    return jsonify({
        ...
        'repaint_guard': {  # 🆕 v8.0
            'enabled': REPAINT_GUARD_ENABLED,
            'min_hold_seconds': MIN_HOLD_SECONDS,
            'block_minutes': REPAINT_BLOCK_MINUTES,
            'suspect_count': len(repaint_suspects),
            'blocked_now': sum(1 for i in repaint_suspects.values()
                               if i.get('until') and datetime.now() < i['until']),
            'anomaly_count': len(anomaly_log)
        },
        'auto_register': {  # 🆕 v7.9
            'enabled': AUTO_REGISTER_SYMBOL,
            'verify_market': AUTO_REGISTER_VERIFY_MARKET,
            'count': len(auto_registered_symbols),
            'max': AUTO_REGISTER_MAX_SYMBOLS,
            'symbols': sorted(auto_registered_symbols)
        },
        ...
        'market_shield_enabled': MARKET_SHIELD_ENABLED,
        'market_shield_running': market_shield_running,
...
def config():
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            # ⚙️ 레버리지가 변경된 심볼은 거래소에 즉시 반영 시도
            leverage_applied = {}
            for symbol, settings in new_config.items():
                if 'leverage' in settings:
                    try:
                        applied = set_leverage(symbol)
                        leverage_applied[symbol] = applied
                    except Exception as lev_err:
                        logger.warning(f"{symbol} 레버리지 거래소 반영 실패: {lev_err}")
                        leverage_applied[symbol] = None
...
def toggle_ai_validation():
    ...
    global SYMBOL_CONFIG, AI_VALIDATION_ENABLED
    try:
        data = request.get_json()
        ...
        # ⚙️ 전역 마스터 스위치 동기화 (대시보드에서 켜면 실제로 검증이 동작하도록)
...
# ============ 🆕 v7.9 Market Shield API ============
@app.route('/market-shield/toggle', methods=['POST'])
def toggle_market_shield():
    """Market Shield ON/OFF"""
    global MARKET_SHIELD_ENABLED
    try:
        data = request.json or {}
        if 'enabled' in data:
            MARKET_SHIELD_ENABLED = bool(data['enabled'])
            status_text = "활성화" if MARKET_SHIELD_ENABLED else "비활성화"
            
            if MARKET_SHIELD_ENABLED and not market_shield_running:
                start_market_shield()
            elif not MARKET_SHIELD_ENABLED and market_shield_running:
                stop_market_shield()
            
            if ENABLE_TELEGRAM:
                emoji = "🛡️" if MARKET_SHIELD_ENABLED else "⛔"
                send_telegram_notification(
                    f"{emoji} <b>Market Shield {status_text}</b>",
                    'success' if MARKET_SHIELD_ENABLED else 'warning'
                )
            
            return jsonify({'status': 'success', 'enabled': MARKET_SHIELD_ENABLED}), 200
        return jsonify({'error': 'Missing enabled field'}), 400
    except Exception as e:
        return jsonify({'error': str(e)}), 500

@app.route('/market-shield/config', methods=['POST'])
def config_market_shield():
    """Market Shield 설정 변경"""
    global MARKET_SHIELD_ENTRY_BLOCK_BEFORE, MARKET_SHIELD_ENTRY_BLOCK_AFTER
    global MARKET_SHIELD_ZSCORE_THRESHOLD, MARKET_SHIELD_ANOMALY_COOLDOWN
    try:
        data = request.json or {}
        if 'block_before' in data:
            MARKET_SHIELD_ENTRY_BLOCK_BEFORE = int(data['block_before'])
        if 'block_after' in data:
            MARKET_SHIELD_ENTRY_BLOCK_AFTER = int(data['block_after'])
        if 'zscore_threshold' in data:
            MARKET_SHIELD_ZSCORE_THRESHOLD = float(data['zscore_threshold'])
        if 'anomaly_cooldown' in data:
            MARKET_SHIELD_ANOMALY_COOLDOWN = int(data['anomaly_cooldown'])
        
        logger.info(f"🛡️ Market Shield 설정 변경: block={MARKET_SHIELD_ENTRY_BLOCK_BEFORE}/{MARKET_SHIELD_ENTRY_BLOCK_AFTER}min, "
                    f"Z={MARKET_SHIELD_ZSCORE_THRESHOLD}σ, cooldown={MARKET_SHIELD_ANOMALY_COOLDOWN}s")
        return jsonify({'status': 'success'}), 200
    except Exception as e:
        return jsonify({'error': str(e)}), 500

@app.route('/market-shield/status', methods=['GET'])
def market_shield_status():
    """Market Shield 상태 조회"""
    return jsonify({
        'enabled': MARKET_SHIELD_ENABLED,
        'running': market_shield_running,
        'block_before_min': MARKET_SHIELD_ENTRY_BLOCK_BEFORE,
        'block_after_min': MARKET_SHIELD_ENTRY_BLOCK_AFTER,
        'zscore_threshold': MARKET_SHIELD_ZSCORE_THRESHOLD,
        'anomaly_cooldown_sec': MARKET_SHIELD_ANOMALY_COOLDOWN,
        'calendar_events': len(market_shield_calendar_cache),
        'calendar_date': market_shield_calendar_date,
        'active_anomalies': {s: {'direction': v['direction'], 'z_price': round(v['z_price'], 1), 'z_volume': round(v['z_volume'], 1)} 
                            for s, v in market_shield_anomaly_flag.items()},
        'blocked_entries': {s: {'reason': v['reason'], 'until': v['until'].isoformat()} 
                          for s, v in market_shield_entry_blocked.items() if datetime.now() < v['until']}
    }), 200
...
def initialize_bot():
    ...
    # 🆕 v8.0: 이상 현상 로그 테이블 초기화
    init_anomaly_table()
    logger.info(f"🛡️ 리페인팅 방어: {'ON' if REPAINT_GUARD_ENABLED else 'OFF'} "
                f"(최소보유 {MIN_HOLD_SECONDS}초 / 차단 {REPAINT_BLOCK_MINUTES}분)")
    ...
    # AI 모니터링 자동 시작
    # ⚙️ 초기 디폴트 OFF: AI_MONITORING_AUTOSTART=True일 때만 자동 시작 (대시보드/API에서 수동 시작)
    if AI_MONITORING_AUTOSTART:
        start_ai_monitoring()
    else:
        logger.info("⚙️ AI 모니터링 자동 시작 비활성화 (초기 OFF). 대시보드에서 수동 시작하세요.")
    
    # 🆕 v7.8: Emergency Drawdown Protection 자동 시작
    if EMERGENCY_DRAWDOWN_ENABLED:
        start_emergency_drawdown_protection()
    else:
        logger.info("⚙️ 긴급 낙폭 보호 비활성화 (초기 OFF). 대시보드에서 수동 시작하세요.")
    
    # 🆕 v7.9: Market Shield 자동 시작
    if MARKET_SHIELD_ENABLED:
        start_market_shield()
    else:
        logger.info("⚙️ Market Shield 비활성화 (초기 OFF). 대시보드에서 수동 시작하세요.")
    ...
    def periodic_data_recording():
        ...
        sync_counter = 0  # 🆕 v7.9: 포지션 동기화 주기 카운터
        ...
        while True:
            try:
                ...
                # 🆕 v7.9: 포지션 동기화 (5분마다)
                # AI monitoring OFF 시에도 바이낸스 포지션 종료 감지 + completed_trades DB 기록
                sync_counter += 1
                if sync_counter >= 1:  # 매 주기(5분)마다
                    sync_counter = 0
                    try:
                        sync_count = sync_positions_from_exchange()
                        if sync_count > 0:
                            logger.info(f"🔄 주기적 포지션 동기화: {sync_count}개 포지션")
                    except Exception as sync_err:
                        logger.warning(f"주기적 sync 오류 (무시): {sync_err}")

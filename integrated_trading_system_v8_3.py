...
def check_overbought_oversold_multi_timeframe(df_15min, df_hourly, df_4h, action):
    """
    다중 시간대에서 과매수/과매도 상태를 체크 - v8.3 강화 버전
    
    Args:
        df_15min: 15분봉 데이터프레임
        df_hourly: 1시간봉 데이터프레임  
        df_4h: 4시간봉 데이터프레임
        action: 'buy' or 'sell'
    ...
    Returns:
        dict: {
            ...
            'warnings': list,
            'scores': dict,
            'reverse_opportunity': bool,
            'reverse_signals': list,
            'total_risk_score': float
        }
    """
    warnings = []
    risk_score = 0  # v8.3: 단일 점수로 변경
    reverse_signals = []
    
    # 🔥 v8.3 핵심 변경: 리스크 임계값 대폭 상향
    HIGH_RISK_THRESHOLD = 7  # 기존 5 → 7
    MEDIUM_RISK_THRESHOLD = 4  # 기존 3 → 4
    
    if action == 'buy':
        # === 15분봉 체크 ===
        rsi_15m = df_15min['rsi'].iloc[-1]
        if rsi_15m > 85:  # 극도의 과매수
            risk_score += 3
            warnings.append(f"🚨 15분봉 RSI EXTREME 과매수 ({rsi_15m:.1f})")
            reverse_signals.append("15m_RSI>85")
        elif rsi_15m > 80:  # v8.3: 75 → 80
            risk_score += 2.5
            warnings.append(f"⚠️ 15분봉 RSI 매우 과매수 ({rsi_15m:.1f})")
        elif rsi_15m > 70:
            risk_score += 1.5
        ...
        if rsi_1h > 80:  # v8.3: 70 → 80
            risk_score += 2.5
            warnings.append(f"⚠️ 1시간봉 RSI 매우 과매수 ({rsi_1h:.1f})")
            reverse_signals.append("1h_RSI>80")
        elif rsi_1h > 75:  # v8.3: 70 → 75
            risk_score += 2
        ...
        if rsi_4h > 75:  # v8.3: 65 → 75
            risk_score += 2
            warnings.append(f"4시간봉 RSI 과매수 ({rsi_4h:.1f})")
            reverse_signals.append("4h_RSI>75")
        elif rsi_4h > 70:  # v8.3: 65 → 70
            risk_score += 1.5
            warnings.append(f"4시간봉 RSI 높음 ({rsi_4h:.1f})")
        elif rsi_4h > 65:
            risk_score += 0.5
        ...
        bb_width = bb_upper_15m - bb_lower_15m
        
        if current_price_15m > bb_upper_15m:
            bb_distance = ((current_price_15m - bb_upper_15m) / bb_width) * 100
            if bb_distance > 10:  # 밴드 폭의 10% 이상 돌파
                risk_score += 2.5
                warnings.append(f"🚨 가격이 볼린저 상단 {bb_distance:.1f}% 돌파")
                reverse_signals.append(f"BB_break_{bb_distance:.0f}%")
            elif bb_distance > 5:
                risk_score += 2
                warnings.append(f"⚠️ 가격이 볼린저 상단 {bb_distance:.1f}% 돌파")
            else:
                risk_score += 1
        ...
        if stoch_k_1h > 90:
            risk_score += 2
            warnings.append(f"🚨 1시간봉 스토캐스틱 극도 과매수 ({stoch_k_1h:.1f})")
            reverse_signals.append("Stoch>90")
        elif stoch_k_1h > 85:  # v8.3: 80 → 85
            risk_score += 1.5
        ...
        # === 멀티 타임프레임 일치성 체크 ===
        overbought_count = sum([rsi_15m > 70, rsi_1h > 70, rsi_4h > 65])
        if overbought_count == 3:
            risk_score += 3
            warnings.append("🚨 모든 타임프레임 과매수!")
            reverse_signals.append("Multi-TF_OB")
        ...
        # === 15분봉 체크 ===
        rsi_15m = df_15min['rsi'].iloc[-1]
        if rsi_15m < 15:  # 극도의 과매도
            risk_score += 3
            warnings.append(f"🚨 15분봉 RSI EXTREME 과매도 ({rsi_15m:.1f})")
            reverse_signals.append("15m_RSI<15")
        elif rsi_15m < 20:  # v8.3: 25 → 20
            risk_score += 2.5
            warnings.append(f"⚠️ 15분봉 RSI 매우 과매도 ({rsi_15m:.1f})")
        elif rsi_15m < 30:
            risk_score += 1.5
        ...
        # === 1시간봉 체크 ===
        rsi_1h = df_hourly['rsi'].iloc[-1]
        if rsi_1h < 20:  # v8.3: 30 → 20
            risk_score += 2.5
            warnings.append(f"⚠️ 1시간봉 RSI 매우 과매도 ({rsi_1h:.1f})")
            reverse_signals.append("1h_RSI<20")
        elif rsi_1h < 25:  # v8.3: 30 → 25
            risk_score += 2
        ...
        # === 4시간봉 체크 ===
        rsi_4h = df_4h['rsi'].iloc[-1]
        if rsi_4h < 25:  # v8.3: 35 → 25
            risk_score += 2
            warnings.append(f"4시간봉 RSI 과매도 ({rsi_4h:.1f})")
            reverse_signals.append("4h_RSI<25")
        elif rsi_4h < 30:  # v8.3: 35 → 30
            risk_score += 1.5
            warnings.append(f"4시간봉 RSI 낮음 ({rsi_4h:.1f})")
        elif rsi_4h < 35:
            risk_score += 0.5
        ...
        if current_price_15m < bb_lower_15m:
            bb_distance = ((bb_lower_15m - current_price_15m) / bb_width) * 100
            if bb_distance > 10:
                risk_score += 2.5
                warnings.append(f"🚨 가격이 볼린저 하단 {bb_distance:.1f}% 돌파")
                reverse_signals.append(f"BB_break_{bb_distance:.0f}%")
            elif bb_distance > 5:
                risk_score += 2
                warnings.append(f"⚠️ 가격이 볼린저 하단 {bb_distance:.1f}% 돌파")
            else:
                risk_score += 1
        ...
        if stoch_k_1h < 10:
            risk_score += 2
            warnings.append(f"🚨 1시간봉 스토캐스틱 극도 과매도 ({stoch_k_1h:.1f})")
            reverse_signals.append("Stoch<10")
        elif stoch_k_1h < 15:  # v8.3: 20 → 15
            risk_score += 1.5
        ...
        macd_1h = df_hourly['macd'].iloc[-1]
        macd_signal_1h = df_hourly['macd_signal'].iloc[-1]
        if action == 'sell' and macd_1h > macd_signal_1h and df_hourly['macd'].iloc[-2] < df_hourly['macd_signal'].iloc[-2]:
    ...
    # v8.3: 반대 진입 기회 (더 엄격한 조건)
    reverse_opportunity = risk_score >= 8 and len(reverse_signals) >= 3
    ...
    # 로그
    if risk_score > 0:
        logger.info(f"🎯 과매수/과매도 체크 - Action: {action}, Score: {risk_score:.1f}, Level: {risk_level}")
        if warnings[:3]:

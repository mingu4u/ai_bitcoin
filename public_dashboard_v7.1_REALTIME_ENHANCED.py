...
def main():
    ...
    with tab2:
        ...
        if not completed_df.empty:
            ...
            for idx, (period_name, days) in enumerate(periods):
                with period_cols[idx]:
                    stats = calculate_period_stats(completed_df, days)
                    ...
                    if stats:
                        ...
                        <div class="period-card">
                            ...
                            <p style="font-size: 0.9rem;">
                                거래: {stats['total_trades']}건<br>
                                승률: {stats['win_rate']:.1f}%<br>
                                ✅ 확인: {stats['binance_verified_rate']:.0f}%
                            </p>
                        </div>
                        """, unsafe_allow_html=True)
                    else:
                        st.info(f"{period_name} 데이터 없음")
            ...
            chart_df = chart_df.sort_values('exit_time')
            chart_df['cumulative_pnl'] = chart_df['pnl_usdt'].cumsum()
            
            fig = go.Figure()
            
            fig.add_trace(go.Scatter(
                x=chart_df['exit_time'],

...
def display_ai_monitoring_section():
    """AI 모니터링 섹션 표시 - v8.3 개선"""
    st.header("🤖 AI Monitoring Status v8.3")
    
    # AI 모니터링 즉시 실행 버튼
    col_btn1, col_btn2, col_btn3 = st.columns([1, 1, 2])
    with col_btn1:
        if st.button("🚀 즉시 AI 모니터링", type="primary"):
            with st.spinner("AI 모니터링 실행 중..."):
                try:
                    response = requests.post(f"{TRADING_BOT_URL}/trigger_ai_monitor")
                    if response.status_code == 200:
                        result = response.json()
                        st.success(
                            f"✅ AI 모니터링 완료!\n\n"
                            f"• 모니터링: {result.get('monitored', 0)}개\n"
                            f"• 종료 결정: {result.get('exits', 0)}개\n"
                            f"• 물타기: {result.get('adds', 0)}개"
                        )
                    else:
                        st.error("모니터링 실행 실패")
                except Exception as e:
    ...
    with col_btn2:
        if st.button("🔄 포지션 동기화"):
            try:
                response = requests.post(f"{TRADING_BOT_URL}/sync_positions")
                if response.status_code == 200:
                    st.success("포지션 동기화 완료")
                else:
                    st.error("동기화 실패")
            except Exception as e:
                st.error(f"오류: {str(e)}")
    ...
    with col1:
        ...
        try:
            ...
            # 물타기 관련 컬럼 확인
            has_add_position_fields = (
                'add_position_margin_percent' in columns and 
                'expected_win_rate' in columns
            )
            ...
            if has_add_position_fields:
                ...
                SELECT 
                    ...
                    urgency,
                    CASE 
                        WHEN trade_type = 'MANUAL_ENTRY' THEN '🔧'
                        WHEN reason LIKE '%Manual position%' THEN '🔧'
                        ELSE '🤖'
                    END as type_icon,
                    action
                FROM trades
            ...
            if not ai_df.empty:
                ...
                if time_diff.total_seconds() < 3600:
                    st.success(f"📊 최근 모니터링: {int(time_diff.total_seconds()/60)}분 전")
                else:
                    st.info(f"📊 최근 모니터링: {latest_time.strftime('%Y-%m-%d %H:%M:%S')}")
                
                # 데이터 포맷팅 - v8.3 색상 이모지 추가
                display_data = []
                for _, row in ai_df.iterrows():
                    # 결정에 따른 색상과 아이콘
                    if row['action'] == 'close':
                        decision_display = '🔴 Close'
                    elif row['action'] == 'partial_close':
                        decision_display = f'🟠 Partial ({row["percentage"]}%)'
                    elif row['action'] == 'hold':
                        decision_display = '🟢 Hold'
                    elif row['action'] == 'add_position':
                        decision_display = '💪 Add'
                    else:
                        decision_display = row['action']
                    
                    # 물타기 정보
                    add_info = ""
                    if row['action'] == 'add_position' and pd.notna(row['add_position_margin_percent']):
                        margin_pct = row['add_position_margin_percent']
                        win_rate = row['expected_win_rate'] * 100 if pd.notna(row['expected_win_rate']) else 0
                        add_info = f"{margin_pct}% (승률 {win_rate:.0f}%)"
                    
                    display_data.append({
                        '시간': row['timestamp'].strftime('%m-%d %H:%M'),
                        '타입': row['type_icon'],
                        '심볼': row['symbol'],
                        '결정': decision_display,
                        '신뢰도': f"{row['confidence']*100:.1f}%" if pd.notna(row['confidence']) else "",
                        '가격': f"${row['price']:,.2f}" if pd.notna(row['price']) else "",
                        '물타기': add_info,
                        '이유': row['reason'][:50] + "..." if len(str(row['reason'])) > 50 else row['reason']
                    })
                ...
                st.dataframe(
                    ...
                    hide_index=True,
                    column_config={
                        "결정": st.column_config.TextColumn(
                            "결정",
                            help="🔴 Close | 🟠 Partial | 🟢 Hold | 💪 Add"
                        ),
                        "물타기": st.column_config.TextColumn(
                            "물타기",
                            help="마진% (예상승률%)"
                        )
                    }
        ...
        try:
            ...
            stats_24h = conn.execute("""
                SELECT 
                    ...
                    SUM(CASE WHEN action = 'partial_close' THEN 1 ELSE 0 END) as partial_decisions,
            ...
            if stats_24h and stats_24h[0] > 0:
                st.metric("📊 24h 모니터링", stats_24h[0])
                
                col_stat1, col_stat2 = st.columns(2)
                with col_stat1:
                    st.metric("🔴 청산", stats_24h[1] or 0)
                    st.metric("🟢 보유", stats_24h[3] or 0)
                
                with col_stat2:
                    st.metric("🟠 부분청산", stats_24h[2] or 0)
                    st.metric("💪 물타기", stats_24h[4] or 0)
                
                st.metric("🎯 평균 신뢰도", f"{stats_24h[5]:.1f}%" if stats_24h[5] else "N/A")
            else:
                st.info("24시간 내 기록 없음")
    ...
    # v8.3 개선사항 정보
    with st.expander("💪 AI 물타기 시스템 v8.3"):
        st.markdown("""
        ### 📖 물타기 시스템 v8.3 개선사항
